...
LightInject_SVOPG::LightInject_SVOPG(pugi::xml_node& featureNode) {
#ifndef NDEBUG
...
void LightInject_SVOPG::init(LightInjectSetting_SVOPG setting) {
	...
	createDescriptorSet();
	Feature::createPipelineLayout(sizeof(shaderio::LightInjectPushConstant_SVOPG));
...
void LightInject_SVOPG::uiRender() {
#ifndef NDEBUG
...
void LightInject_SVOPG::resize(VkCommandBuffer cmd, const VkExtent2D& size) {
...
void LightInject_SVOPG::preRender() {
...
void LightInject_SVOPG::render(VkCommandBuffer cmd) {
	NVVK_DBG_SCOPE(cmd);
...

	{
		...
		const VkPushConstantsInfo pushInfo{
			...
			.layout = rtPipelineLayout,
			.stageFlags = VK_SHADER_STAGE_ALL,
			.size = sizeof(shaderio::LightInjectPushConstant_SVOPG),
...
void LightInject_SVOPG::postProcess(VkCommandBuffer cmd) {
#ifndef NDEBUG
	debug_Cube(cmd);
#endif
}

void LightInject_SVOPG::createDescriptorSetLayout() {
	...
	bindings.addBinding({
			...
			.stageFlags = VK_SHADER_STAGE_ALL });
	bindings.addBinding({
		.binding = shaderio::StaticBindingPoints_LightInject_SVOPG::eVGB_LightInject_SVOPG,
...
void LightInject_SVOPG::createDescriptorSet() {
	nvvk::WriteSetContainer write{};
	...
	VkWriteDescriptorSet    VGBWrite =
		staticDescPack.makeWrite(shaderio::StaticBindingPoints_LightInject_SVOPG::eVGB_LightInject_SVOPG, 0, 0, setting.VGBs.size());
	nvvk::Buffer* VGBsPtr = setting.VGBs.data();
	write.append(VGBWrite, VGBsPtr);

	vkUpdateDescriptorSets(Application::app->getDevice(), write.size(), write.data(), 0, nullptr);
}
void LightInject_SVOPG::createPipeline() {
	SCOPED_TIMER(__FUNCTION__);
	...
	const VkPushConstantRange push_constant{ VK_SHADER_STAGE_ALL, 0, sizeof(shaderio::LightInjectPushConstant_SVOPG) };
...
void LightInject_SVOPG::compileAndCreateShaders() {
	createPipeline();
...
void LightInject_SVOPG::updateDataPerFrame(VkCommandBuffer cmd) {
...
#ifndef NDEBUG
void LightInject_SVOPG::debug_Cube(VkCommandBuffer cmd) {
	NVVK_DBG_SCOPE(cmd);
