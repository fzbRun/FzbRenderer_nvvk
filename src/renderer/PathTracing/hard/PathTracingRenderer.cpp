...
void FzbRenderer::PathTracingRenderer::rayTraceScene(VkCommandBuffer cmd) {
	...
	write.append(rtDescPack.makeWrite(shaderio::BindingPoints::eTlas), tlasAccel);
	...
	vkCmdTraceRaysKHR(cmd, &raygenRegion, &missRegion, &hitRegion, &callableRegion, size.width, size.height, 1);
