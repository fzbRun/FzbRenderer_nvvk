...
ImageRecognition::ImageRecognition(ImageRecognition_CreateInfo createInfo) {
	...
	inputTensorExtMem = importVulkanMemoryObjectFromNTHandle(buffer.handle, buffer.allocMemTotalSize, false);
	inputTensor = (float*)mapBufferOntoExternalMemory(inputTensorExtMem, buffer.allocMemOffset, buffer.allocMemSize);
	...
	resNet34 = std::move(FzbRenderer::Model(createInfo.modelCreateInfo));
}
...
void ImageRecognition::infer(uint32_t frameIndex, uint64_t waitTimeline) {
